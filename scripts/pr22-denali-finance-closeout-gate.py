#!/usr/bin/env python3
"""PR22 — Denali Finance first-customer closeout & production observation gate."""
...
JAR = Path(os.environ.get("SMOKE_COOKIE_JAR", "/tmp/pr22.jar"))
OUT = Path(os.environ.get("SMOKE_RESULTS", "/tmp/pr22-closeout.json"))
SAFETY_OUT = Path(os.environ.get("SMOKE_SAFETY", "/tmp/pr22-safety.json"))
OBS_OUT = Path(os.environ.get("SMOKE_OBS", "/tmp/pr22-observation.json"))
TS = time.strftime("%Y%m%d%H%M%S", time.gmtime())
JPG = Path("/tmp/pr22.jpg")
...
def curl(
    ...
    *,
    ...
    if raw_body is not None:
        data_file = Path("/tmp/pr22-req.bin")
        data_file.write_bytes(raw_body)
        cmd += ["--data-binary", f"@{data_file}"]
    elif body is not None:
        data_file = Path("/tmp/pr22-req.json")
    ...
    raw = Path("/tmp/pr22-body.bin").read_bytes()
...
def login() -> None:
    ...
    ch = json.loads(Path("/tmp/pr22-body.bin").read_text() if False else "null")
...
def ensure_fresh_registration() -> str:
    ...
    # Create operator bookings on published tours, then approve
    code_t, tours, _ = get_json("/api/tours?limit=20")
    tour_items = (tours or {}).get("items") or [] if isinstance(tours, dict) else []
    for t in tour_items:
        if not t.get("departureAt"):
            continue
        guest = f"PR22-{uuid.uuid4().hex[:12]}"
        c, resp, ms, _ = curl(
            "POST",
            "/api/bookings",
            body={
                "tourId": t["id"],
                "tourTitle": t.get("title") or "Tour",
                "guestLabel": guest,
                "partySize": 1,
                "departureAt": t.get("departureAt"),
            },
        )
        bid = resp.get("id") if isinstance(resp, dict) else None
        if c not in (200, 201) or not bid:
            continue
        ac, aresp, ams, _ = curl("POST", f"/api/bookings/{bid}/approve")
        if ac == 200:
            result["ids"]["registration"] = bid
            step(
                "fixture_create_approve_for_fresh",
                "created+approved unpaid registration",
                bid,
                "PASS",
                http=ac,
                latency_ms=ams,
                evidence=f"tour={t.get('id')} guest={guest}",
            )
            return bid
...
def seed_payment_receipt(reg: str, amount: str, tag: str) -> tuple[str, str]:
    ...
    code, pay, ms, text = curl(
        ...
        headers={"Idempotency-Key": f"pr22-pay-{tag}-{TS}"},
    )
...
def main() -> int:
    ...
    code, pay, ms, text = curl(
        ...
        headers={"Idempotency-Key": f"pr22-pay-u1-{TS}"},
    )
    ...
    dup_code, dup_body, dup_ms, _ = curl(
        ...
        headers={"Idempotency-Key": f"pr22-dup-{TS}"},
    )
    ...
    if clean_s:
        ...
        cmd_body = {
            ...
            "correlationId": f"pr22-stale-{TS}",
            "reviewReceipt": {
                "registrationId": reg_s,
                "counterpartyId": reg_s,
                "receiptId": rid_s,
                "reviewNote": "PR22 stale",
            },
        }
        if fp_s:
        ...
        stale_code, stale_body, stale_ms, _ = curl(
            ...
            body=cmd_body,
            headers={"Idempotency-Key": f"pr22-stale-cmd-{TS}"},
        )
    ...
    step(
        ...
        http=xt_code,
        evidence=Path("/tmp/pr22-xt.json").read_text(errors="ignore")[:300],
    )
    if xt_code == 200:
        find(
            "BLOCKER",
            "Cross-tenant finance list returned 200 on urban host with denali session",
            Path("/tmp/pr22-xt.json").read_text(errors="ignore")[:400],
            "tenant-isolation",
        )
    ...
    # Production observation from this controlled smoke window (existing fail-open route timings)
    routes = result.get("routes") or []
    lat = sorted(float(r.get("ms") or 0) for r in routes if r.get("ms") is not None)
    def pct(p):
        if not lat:
            return None
        i = min(len(lat) - 1, max(0, int(round((p / 100.0) * (len(lat) - 1)))))
        return round(lat[i], 1)
    statuses = [int(r.get("status") or 0) for r in routes]
    obs = {
        "source": "controlled_closeout_smoke_window",
        "sampleSize": len(routes),
        "requestCount": len(routes),
        "availability": round(sum(1 for s in statuses if 200 <= s < 500) / len(statuses), 4) if statuses else None,
        "latencyMs": {"p50": pct(50), "p95": pct(95), "p99": pct(99)},
        "commandSuccessFailure": "INSUFFICIENT_SAMPLE",
        "staleRate": "INSUFFICIENT_SAMPLE",
        "authFailures": sum(1 for s in statuses if s == 401),
        "providerDegradation": "INSUFFICIENT_SAMPLE",
        "incompleteReadings": "INSUFFICIENT_SAMPLE",
        "exceptionRate": "INSUFFICIENT_SAMPLE",
        "meaningUnavailableTimeout": "INSUFFICIENT_SAMPLE",
        "operatorReturnToOperationalView": "INSUFFICIENT_SAMPLE",
        "sotMutationFailures": sum(1 for s in result["steps"] if s["status"] == "FAIL" and "failure_" in s["step"]),
        "productionTrafficVolume": "INSUFFICIENT_SAMPLE",
        "note": "No sustained real-customer production volume; metrics from controlled Denali closeout smoke only.",
    }
    result["observation"] = obs
    OBS_OUT.write_text(json.dumps(obs, indent=2))

    # Known non-blockers reaffirmed (do not auto-fix)
    find(
        "UX_IMPROVEMENT",
        "Overview may still reference prepay/installments while Denali hides those tabs",
        "Accepted first-customer limitation; not material to core payment/receipt journey",
        "known-nonblocker-1",
    )
    find(
        "DOCUMENTATION",
        "open-payments / prepayments / installments remain out of first-customer scope",
        "Do not implement unless customer requirement appears",
        "known-nonblocker-3-4",
    )

    if blockers or failed_core or material_risks:
        result["verdict"] = "BLOCKED" if (blockers or failed_core) else "CONTINUE_OBSERVATION"
        if material_risks and not blockers and not failed_core:
            # material CUSTOMER_RISK without core fail → cannot close
            result["verdict"] = "BLOCKED"
    else:
        # Delivery closeout: journey+safety proven. Observation volume is explicitly INSUFFICIENT_SAMPLE
        # but does not reopen architecture or block first-customer delivery closeout.
        result["verdict"] = "READY_FOR_FINANCE_CLOSEOUT"
    ...
    result["summary"] = {
        ...
        "observationSample": obs["sampleSize"],
        "productionVolume": "INSUFFICIENT_SAMPLE",
    }
    # Safety extract
    SAFETY_OUT.write_text(json.dumps({
        "verdict": result["verdict"],
        "safetySteps": [s for s in result["steps"] if s["step"].startswith("failure_") or "ui_sot" in s["step"] or "meaning" in s["step"]],
        "findings": result["findings"],
    }, indent=2))
    save()
    print("VERDICT", result["verdict"])
    print("findings", result["summary"]["findings"])
    print("observation", obs["requestCount"], "requests; productionVolume=INSUFFICIENT_SAMPLE")
    return 0 if result["verdict"] == "READY_FOR_FINANCE_CLOSEOUT" else 1
