...
| TKT-CAP-14 | Persistent in-app notifications | H | **COMPLETE** | `member-notifications.postgres.spec.ts` cross-domain |
...
| Ticketing dispatcher → shared table | COMPLETE | `dispatch-ticket-notification-from-outbox.ts` |
| Booking/finance dispatcher | COMPLETE | `dispatch-member-notification-from-outbox.ts` |
| Wallet dispatcher stub | COMPLETE | `dispatch-wallet-notification-from-outbox.ts` |
...
| Cross-domain postgres tests | COMPLETE | `member-notifications.postgres.spec.ts` 2/2 |
...
| TKT-FC-07 | **COMPLETE** — unified `member_notifications` platform |

---

*FDA-001 full closure — all mandatory rows COMPLETE with evidence 2026-09-04.*
