...
| GAP-3.3-06  | Wizard `composite`                                 | Read-only unsupported UI                                         | 6 (widget resolver) |
| GAP-3.3-06b | Wizard `number` / `date`                           | Wired via `@app-tour/ui-primitives/input` (`type=number          | date`)              | **Closed** (3.3 shell) |
