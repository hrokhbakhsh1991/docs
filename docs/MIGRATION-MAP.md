...
### ۳.۴.۱ Guest plugin onboarding (PF-0 → PF-4)

New guest-capable workspace **must not** hand-edit SDK resolver maps, portal shell, or `workspace-plugin-host` wiring.
...
| ----------------------- | ----------------------------------------------------- |
| **CASL**                | Authorization — _who_ may load workspace brand tokens |
| **Theme Ingress Guard** | Validation — _what_ CSS is safe if loaded             |
...
> **Phase 5 pack (authoritative detail):** [`docs/phase-5/appendices/test-inventory.md`](phase-5/appendices/test-inventory.md) · [`docs/phase-5/phase-5-agent-router.md`](phase-5/phase-5-agent-router.md). Until subphase **5.4** is VERIFIED_BEHAVIORAL, `phase-5.contract.spec.ts` is **SCAFFOLD** (file/DDL existence only) — **not** outbox relay proof.

- [ ] **Contractual Gate:** No grep/regex **required** check for Phase 5; must implement `phase-5.contract.spec.ts` at **`apps/api/test/phase-5.contract.spec.ts`** (scaffold at 5.6; **outbox + repository boundaries** require behavioral specs per test-inventory §5.4, not contract alone).
...
| **Contract**               | `apps/api/test/phase-5.contract.spec.ts` (scaffold gate; behavioral outbox: subphase 5.4 tests) |
...
**Phase 7 PEK (doc pack):** [`phase-7-platform-dod.md`](phase-7-platform-dod.md) · [`phase-7/phase-7-agent-router.md`](phase-7/phase-7-agent-router.md) · guard: `pnpm run phase-7:guard` · doc score target **≥96**.
