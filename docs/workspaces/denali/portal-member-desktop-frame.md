...
Inspired by Linear / Vercel account settings and Pocket 3.5 canvas language — **not** stacked gradient cards:

| Mobile `<48rem` | Desktop `≥48rem` |
| --------------- | ---------------- |
| Canvas title + grouped white sections + sticky Save | **Sectioned form**: hairline dividers, no nested card boxes, no hero/photo orb |
| Single column fields | 2-column field grid; mobile OTP full-span |
| Sticky full-width solid forest Save | **Actions footer**: Discard + Save, end-aligned, no glass tray |
...
- `data-member-profile-session` — mobile logout row (hidden on desktop; rail footer owns sign-out)
...
| `portal/member-profile.css` | Pocket 3.5 grouped account + **PS-VIS-5g sectioned** desktop |
...
Manual: 1440×900 — side rail visible, **no floating phone-card**, mist canvas; profile **sectioned** (no hero gradient); 390px compact header + thumb bar + sticky save.
