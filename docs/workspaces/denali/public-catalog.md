...
Denali and Urban exposure resolvers (`resolve-denali-surface-exposure.ts`, `configure-urban-surface-exposure.ts`) catch Prisma failures when `DATABASE_URL` is unset and fall back to registry-seeded field defaults — required for DB-less Playwright smokes (SMK-MKT-03 portal register, SMK-MKT-05 urban catalog).
...
| Urban exposure resolver (API) | `apps/api/src/http/configure-urban-surface-exposure.ts` |
