...
gate_json:
  ...
  full_pass: "phase-5:gate exit 0 — includes direct phase-4:guard; phase-4:gate is a separate same-SHA prerequisite"
```
...
closure_gate:
  ...
  chain: "pnpm run db:test-reset && pnpm build && pnpm test && pnpm run phase-4:guard && pnpm run phase-5:guard"
...
| ---- | -------------------------------------------------------------- | ------------- | --------------------------------------- |
| 1    | `pnpm run db:test-reset`                                       | 5.0           | DEC-095, REQ-P5-002                      |
| 2    | `pnpm build`                                                   | all           | —                                       |
| 3    | `pnpm test`                                                    | all           | REQ-P5-024 (partial)                    |
| 4    | `pnpm run phase-4:guard`                                       | 5.0           | REQ-P5-002, RULE-034                    |
| 5    | Migration up (`DATABASE_URL`)                                  | 5.1, 5.4, 5.5 | REQ-P5-007,015,023                      |
| 6    | `pnpm --filter @apps/api test`                                 | 5.2–5.5       | REQ-P5-009–023                          |
...
| 7    | `pnpm run phase-4:gate` (same-SHA prerequisite, run separately) | 5.6           | REQ-P5-001–006, REQ-P5-039               |
| 8    | `pnpm run phase-5:gate`                                        | 5.6           | REQ-P5-039                              |
