...
| Ops UI finance manifest | `finance-ops-panels.ts` may import Denali **panel** manifest for Denali hub layout; **availability** SoT is `finance-nav-enablement` → generated nav bindings only (no Denali) |
