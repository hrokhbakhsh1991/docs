...
phase_2_gate:
  ...
  steps_ordered:
    - step: 0
      run: pnpm run check:node-engine
    - step: 1
      run: pnpm build
      includes:
    ...
    - step: 2
      run: pnpm test
      includes: "monorepo package tests incl. design-tokens + platform-core regression PC-1"
    - step: 2b
      run: pnpm --filter @app-tour/platform-core run test:phase-2
      guard_id: p2_phase2_contract_behaviors
      note: "Explicit in gate script; also enforced inside phase-2:guard"
    - step: 3
...
ci_integrity_pre_commit:
  ...
  note: "Full phase-0 + phase-1 + phase-2 gates on every commit (DRIFT-P2-11 closed)"
...
github_workflow:
  ...
  steps:
    ...
    - pnpm run phase-2:gate
  note_engine: "check-node-engine is step 0 inside phase-2:gate"
