...
| --- | --- | --- |
| Equipment name | `Smoke Trekking Poles` | `عصای کوهنوردی` (+ camping: `چادر`, `کیسه خواب`) |
| Theme name | `Smoke Mountain` | `کوهستان` (+ `طبیعت / کمپینگ`, `formProfile: nature_trip`) |
| Theme slug | `smoke-mountain` | `smoke-mountain` (stable id/FK); camping slug `nature-camping` |
| Destinations / region | English smoke | already FA (`توچال` / `تهران`) |

**ED-THEME-CAMP-01:** nature tours hide `mountain_outdoor` themes. Club seed therefore adds a `nature_trip` theme and tent/sleeping-bag rows (`category: nature`) under ids `…707` / `…708` / `…709`. Operator smoke `…014` is unchanged.
