...
`WorkspaceWizardHost.resolveInitialStepIndex` infers the furthest step with **meaningful user-entered** field data when `meta.currentStepIndex === 0`. Sanitize/template scalars (`false`, `"false"`, `"none"`, `publishStatus` `draft`, etc.) are ignored via `isPhantomCanonicalScalar` in `resolve-initial-step-index.ts` so Branch B phantom-default inference cannot jump to pricing. Explicit operator values such as `difficultyLevel` `5` or `fitnessLevel` `medium` **do** count (they are no longer auto-seeded).
...
**Step-nav required for composite dependents (INV-DENALI-WIZ-005 + INV-WIZ-002):** `validateDenaliWizardDraftSync` expands `DENALI_COMPOSITE_DEPENDENTS_BY_ANCHOR` into the active step plan before `mergeDenaliStepRequiredFieldViolations`. Synthetic dependent rows must copy **`ruleDefaults.required` / `ruleDefaults.hidden`** from the Denali field registry (e.g. `program.shortDescription` → `required: true`). Hard-coding `required: false` on expand lets Continue leave `denali_photos` with an empty short description — a regression against the matrix.

**Step-nav empty composites (INV-DENALI-WIZ-009):** `isDenaliDraftFieldValueEmpty` must treat empty arrays as empty (e.g. `leaderUserIds`, `program.themeIds`) and for **`program.itinerary`** must also treat zero days / days failing `collectDenaliItineraryDayValidationIssues` as empty. Otherwise multi-day `denali_program` Continue succeeds with `itinerary: []` even when contextual rules mark itinerary `required: true`.

**Itinerary row sync vs required content:** `syncDenaliItineraryRows` (structural `syncProgramItineraryToDayCount`) may mint day shells so multi-day draft length matches `start`/`end`. Scaffold rows must use **empty** day/segment titles (same as `buildDefaultItineraryDays`), never English placeholders like `Day N` / `Activity N`. Sanitize runs before step-nav validation when `evalContext` is present; placeholder titles would falsely satisfy `collectDenaliItineraryDayValidationIssues` and let Continue pass without operator input. Catalog egress may still fall back to `Day N` for display-only projection when a published title is blank.

**Itinerary labels + field-level invalid (a11y):** The itinerary **composite header** uses `composites.itinerary.sectionTitle`. Validation summary rows resolve through `resolveDenaliValidationIssueLabel` (INV-DENALI-WIZ-018) — for `denali.itinerary` that maps to the leaf `fields.program.itinerary`, which is kept in sync with the same “Day-by-day itinerary” / «برنامه روزبه‌روز» copy. When step-nav marks the itinerary invalid, `DenaliItineraryField` applies `aria-invalid` / `invalid` only to day title and segment title controls that fail `collectDenaliItineraryDayValidationIssues` for the displayed rows — not every input in the matrix.

**Validation issue labels prefer leaf over group sectionTitle (INV-DENALI-WIZ-018):** Step-nav / review issue `path` may be a composite renderer id when the empty required leaf is a composite **anchor** (`participants.minimumAge` → `denali.pricing-participants`). Label resolution must match `resolveDenaliFieldLabel`: map via `DENALI_COMPOSITE_LABEL_CANONICAL_PATH` to the actionable leaf (`fields.participants.minimumAge` → «حداقل سن»), not the composite group `sectionTitle` («الزامات شرکت‌کننده»). Unmapped composite widgets still fall back to `sectionTitle`. Continue stays validate-on-click (not hard-disabled); when issues are open the host may set soft Continue attention styling without blocking retry.

**Composite expand + contextual required (INV-DENALI-WIZ-010):** After synthesizing `DENALI_COMPOSITE_DEPENDENTS_BY_ANCHOR` rows for step-nav required checks, re-run `applyDenaliConditionalFieldRules` on the expanded step with the current draft. Expand alone only copies `ruleDefaults.required` — that misses `contextualRequired` (e.g. `pricing.basePricePerPerson` when `pricing.requiresPayment` is true) and would let Continue leave `denali_pricing` with an empty price while the payment composite shows the required input.

**Location zones UI density (INV-DENALI-WIZ-019):** Do not remove `startPoint` / `summitPoint` / `campPoint` / `endPoint` from the registry or nature matrix to shorten logistics. Collapse empty zone `<details>` and defer map mount until the operator opens the panel — see [`docs/workspaces/denali/wizard-experience.md`](../workspaces/denali/wizard-experience.md) (Composites).

**Expand must forward `evalContext` (INV-DENALI-WIZ-012):** Host step-nav already evaluated the visible plan with tenant `uiOptions` + `fieldRulesOverlay` (`ruleSet`). Re-applying conditionals on the expanded step without that same `evalContext` re-resolves against the bare matrix and can resurrect overlay-hidden dependents (false `REQUIRED_FIELD_EMPTY`) or drop overlay-required leaves. `ExpandCompositeDependentsOptions` / `validateDenaliWizardDraftSync` must pass `evalContext` into `applyDenaliConditionalFieldRules` identically to `wizardHost.applyContextualFieldRules`.

**Host Continue must pass `evalContext` (INV-DENALI-WIZ-014):** `WorkspaceWizardHost.handleBeforeNext` calls `wizardHost.validateDraftSync` for step-nav. That call must include the same `wizardRuleEvalContext` already used for review refresh and contextual plan eval. Omitting it means Continue validates against a bare matrix while expand (INV-012) and review use overlays — tenant-hidden required leaves can false-block, or overlay-required leaves can false-pass. Live review already passed `evalContext`; Continue must match.

**Step-nav summary clear (INV-DENALI-WIZ-015):** Do not clear step-nav validation issues on draft **reference** change alone. Compare `stableWizardDraftDataKey(draft)` (`JSON.stringify(data)`); clear only when payload changes or `activeStepIndex` changes. Same-payload remints (resume merge / `prepareEnvelope`) must keep the alert and `aria-invalid` until the operator edits or leaves the step.

**Shared surface a11y + step heading (INV-DENALI-WIZ-017):** `invalid` / `validationIssuePaths` live in `@app-tour/wizard-navigation` as `WizardCompositeA11yProps` — web and Denali `wizard-surface-types` extend that contract (do not fork the a11y fields). Step-nav Continue must pass `validationHeadingKey: "review.stepValidationHeading"` into the Denali validation summary so operators see «قبل از ادامه…» / “Fix these issues before continuing”, not the create-tour heading. Review / submit keep `review.validationHeading`. Draft types may still differ per package (`TourWizardDraft` vs `DenaliTourWizardDraft`); that split is intentional thin-shell boundary.

**Transport composite expand (INV-DENALI-WIZ-013):** `transport.dongAmount` (`contextualRequired: transportDongVisible`) and `transport.seatPreference` (`contextualRequired: transportTrainSeatVisible`) render inside `denali.transport-mode` but were missing from `DENALI_COMPOSITE_DEPENDENTS_BY_ANCHOR`. Step-nav expand therefore never synthesized those rows, so Continue on `denali_logistics` succeeded with `shared_cars` + empty dong (or `train` + empty seat) while the composite UI marked the leaf required. Transport dependents must live on the same SoT map as pricing/program anchors (not only the template-overlay / review hardcodes). Create-submit merge of canonical + publish-readiness must `dedupeValidationViolations` the same way live review does.

**`transport.seatPreference` Layer C vs wizard (INV-DENALI-WIZ-016):** Registry `settingsSurface: "deprecated"` means **Settings wizard-template builder** excludes the path (INV-WIZ-002 / `wizard_overlay_exclude`) — operators cannot toggle it as a standalone template overlay leaf. It does **not** mean the create wizard omits the control. `DenaliTransportModeField` still shows seat preference when mode is `train`, and `contextualRequired: transportTrainSeatVisible` + composite expand (INV-013) keep Continue honest. Registry notes must not say “no modern wizard input.” Add `structuralInvariant: clearWhenNotVisible` so leaving train clears a stale seat leaf (same class as `transport.dongAmount`).

**Review / submit issue → step links (INV-DENALI-WIZ-011):** Composite dependents are omitted from the visible render plan and from the tenant template field list (INV-WIZ-002 / INV-DENALI-WIZ-006). `buildFieldStepResolver(visibleSteps)` and `buildFieldStepResolverFromTemplate(templateSteps)` therefore cannot attach `stepId` for paths like `program.shortDescription`, `pricing.basePricePerPerson`, or `participants.fitnessLevel`. Mapping must fall back to Denali field-registry `stepId` (via `buildFieldStepResolver*` registry append + host `wizardHost.resolveValidationStepId`). Otherwise review validation groups show under `"unknown"` and `scrollToFirstIssue` skips `goToStep`. Registry fill is **gap-only**: plan/template entries already in the map win (`!has` guard). Anchors remapped by a tenant template therefore keep the template `stepId`; only paths absent from the plan (true composite dependents) take registry `stepId`.

**Fitness / difficulty — no phantom defaults (INV-DENALI-WIZ-010):** `participants.fitnessLevel` (matrix-required for mountain, inside `denali.pricing-participants`) and `program.difficultyLevel` (matrix-required outdoor program) must stay **empty until the operator chooses a value**. Do **not** use `structuralInvariant: defaultWhenVisible` to seed `"medium"` or `5` on sanitize — that made the UI look filled while treating the leaf as user progress / completion. Empty draft → Select placeholder («سطح آمادگی را انتخاب کنید…») / difficulty unset label; required validation fails until set. Unset difficulty **thumb** parks at min (`DIFFICULTY_LEVEL_SLIDER_UNSET_POSITION = 1`) with empty fill — not mid-5 — so the control does not look pre-selected; the first operator `input` / unset `pointerup` gesture commits a real value (ED-DIFF-01). Operator-chosen `5` / `medium` are real values (not phantoms for resume inference). Legacy `defaultWhenVisible` dependents that still exist elsewhere may use `shouldPersistCanonicalPathFromForm` exception; fitness/difficulty no longer need it.
...
**Step-nav `aria-invalid` for dependents:** Host `invalid` is keyed by the render-plan canonical path (anchor). Composite widgets also receive `validationIssuePaths` (issue `path` list) so dependents inside the widget (e.g. short description textarea) can set `aria-invalid` when the issue targets the dependent path, not only the anchor.

**Rule-eval form projection (INV-DENALI-WIZ-007):** `tourWizardDraftToDenaliForm` maps **all** canonical draft leaves into the legacy form for rule evaluation and publish readiness — including composite dependents stored only on the draft envelope (e.g. `program.shortDescription` → `programNature.shortDescription`). `shouldPersistCanonicalPathFromForm` still applies only on **form→draft** sanitize/sync so composite anchors remain the write SoT (with the INV-DENALI-WIZ-010 `defaultWhenVisible` exception); skipping dependents on draft→form caused false `VALIDATION_RULE_REQUIRED_FIELD` on active publish when the photos textarea was filled.
