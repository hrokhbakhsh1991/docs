...
**Fitness level default (INV-DENALI-WIZ-010):** `participants.fitnessLevel` is matrix-required for mountain participants and is rendered inside `denali.pricing-participants`. Registry `structuralInvariant: { kind: "defaultWhenVisible", value: "medium" }` must seed the draft on sanitize so the Select’s displayed default matches canonical storage (same class of bug as difficulty’s `defaultWhenVisible: 5`). Empty string is treated as missing for `defaultWhenVisible` (not only `null`/`undefined`). Composite dependents normally skip form→draft persist (`shouldPersistCanonicalPathFromForm`), but **`defaultWhenVisible` dependents are an exception** so the seeded leaf is written back to the draft envelope. UI must not show a phantom `medium` while the draft leaf is empty — use Select placeholder until sanitize writes the leaf.
