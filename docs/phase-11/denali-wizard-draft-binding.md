...
**Rule-eval form projection (INV-DENALI-WIZ-007):** `tourWizardDraftToDenaliForm` maps **all** canonical draft leaves into the legacy form for rule evaluation and publish readiness — including composite dependents stored only on the draft envelope (e.g. `program.shortDescription` → `programNature.shortDescription`). `shouldPersistCanonicalPathFromForm` still applies only on **form→draft** sanitize/sync so composite anchors remain the write SoT; skipping dependents on draft→form caused false `VALIDATION_RULE_REQUIRED_FIELD` on active publish when the photos textarea was filled.
