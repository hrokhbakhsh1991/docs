...
**Itinerary labels + field-level invalid (a11y):** Validation summary and the composite header both use `composites.itinerary.sectionTitle` (not the legacy `fields.program.itinerary` “summary” wording). When step-nav marks the itinerary invalid, `DenaliItineraryField` applies `aria-invalid` / `invalid` only to day title and segment title controls that fail `collectDenaliItineraryDayValidationIssues` for the displayed rows — not every input in the matrix.
