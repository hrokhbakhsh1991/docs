...
| SMK-MKT-03 / SMK-P8 guest OTP after `register-complete` | `resolveSmokeApiJwtEnv()` mints a **new** RS256 pair every smoke-script process. Reusing API/portal on `:3001`/`:3003` from a prior run signs `atour_mb_session` with key A while this process injects key B into portal/web → middleware `invalid_signature` → cookie cleared → guest OTP loop. `smoke-marketing-e2e-servers.mjs` and `smoke-urban-e2e-servers.mjs` **must** `fuser -k` 3000–3003 and spawn API+surfaces with the **same** `jwtEnv`. Spread `jwtEnv` into API, portal, web, **and** marketing (SSR `validateSessionTokenAsync`). Do not `spawn` API with `--env-file=.env.local` if that file holds a different `AUTH_JWT_PUBLIC_KEY`. |
