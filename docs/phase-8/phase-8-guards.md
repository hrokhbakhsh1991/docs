...
| Script                                                    | Role                                                                                          |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `pnpm run phase-8:guard`                                  | Phase 8 charter gates (24 checks · doc + invariants + hardening + Sprint F–M)                 |
...
| 5   | `p8_doc_hardening`                | yes      | `verifyDocHardening` (`phase-8-doc-hardening.mjs`)                  | All **33** canonical PEK paths under `docs/phase-8/` exist on disk                                                                                                                                            |
...
| 10  | `p8_spec_path_registry`           | yes      | `verifySpecPathRegistry` (`phase-8-hardening-artifacts.mjs`)        | **6** paths: 4× `apps/api/test/urban-*.spec.ts` · `packages/workspace-sdk/test/urban-owner-ability.spec.ts` · `apps/web/test/urban-owner-access.spec.ts` — each with `describe` / `it` / `expect().toBe()`    |
...
| 12  | `p8_truth_attestation_sync`       | yes      | `verifyTruthAttestationSync` (`phase-8-hardening-artifacts.mjs`)    | No stale **9/9** attestation · `IMPLEMENTATION-TRUTH` cites `19/19 PASS` and `charter_gates: 24`                                                                                                              |
| 13  | `p8_prove_with_parity`            | yes      | `verifyProveWithParity` (`phase-8-hardening-artifacts.mjs`)         | `SPEC-REGISTRY-8.1.yaml` ↔ guard registry ↔ BOOT-MANIFEST 8.1 ↔ subphase 8.1 ↔ truth ↔ verification-matrix                                                                                                    |
| 14  | `p8_api_surface_alignment`        | yes      | `verifyApiSurfaceAlignment` (`phase-8-hardening-artifacts.mjs`)     | **DEC-P8-004** · SDK `authz.canPerformUrbanOwnerMutation` · `isWorkspaceOwner` from tenant-auth-grants · router urban-settings-access                                                                         |
| 15  | `p8_envelope_spec_depth`          | yes      | `verifyEnvelopeSpecDepth` (`phase-8-hardening-artifacts.mjs`)       | `urban-settings-patch.spec.ts` ASM-001 asserts `metadata.correlationId` · `primaryColor` · `featureFlags` · `rateLimitRps`                                                                                    |
| 16  | `p8_entry_ledger_present`         | yes      | `verifyEntryLedgerPresent` (`phase-8-hardening-artifacts.mjs`)      | `reports/phase-8-entry-verified.yaml` exists · `phase_7_gate` · `map_22_reviewed` · `status: PENDING\|PASS` · PASS requires `exit_code: 0`                                                                    |
| 17  | `p8_owner_auth_specs`             | yes      | `verifyOwnerAuthSpecs` (`phase-8-charter-deferred.mjs`)             | CASL 8.1 `GET/PATCH /urban/settings` rows ↔ SDK/API/WEB case IDs on disk                                                                                                                                      |
| 18  | `p8_urban_routes_bound`           | yes      | `verifyUrbanRoutesBound` (`phase-8-charter-deferred.mjs`)           | Route matrix §C settings paths ⊆ dispatch addendum · 8.2 paths in `out_of_scope_8_1` only                                                                                                                     |
| 19  | `p8_smoke_map_present`            | yes      | `verifySmokeMapPresent` (`phase-8-charter-deferred.mjs`)            | SMK-P8-01..04 each has executable command in verification-matrix smoke index                                                                                                                                  |
| 20  | `p8_verification_matrix_hydrated` | yes      | `verifyVerificationMatrixHydrated` (`phase-8-charter-deferred.mjs`) | REQ-P8-010..012 cite spec anchors + on-disk test paths                                                                                                                                                        |
| 21  | `p8_boundary_ci_hook`             | yes      | `verifyBoundaryCiHook` (`phase-8-charter-deferred.mjs`)             | `p8-boundary-diff.mjs` on disk · documented in guards + PHASE-BOUNDARY-MATRIX                                                                                                                                 |
| 22  | `p8_no_legacy_runtime_import`     | yes      | `runP8NoLegacyRuntimeImport` (`phase-8-guard.mjs`)                  | Zero legacy imports in urban scan roots                                                                                                                                                                       |
| 23  | `p8_urban_not_denali_rail`        | yes      | `runP8UrbanNotDenaliRail` (`phase-8-guard.mjs`)                     | Urban ↔ denali isolation                                                                                                                                                                                      |
| 24  | `p8_technical_quality`            | yes      | `runP8TechnicalQuality` (`phase-8-guard.mjs`)                       | TQ-P8-001..010 in charter                                                                                                                                                                                     |
...
Verified baseline artifact (Sprint F–L): [`reports/phase-8-gate-2026-06-07.json`](../../reports/phase-8-gate-2026-06-07.json) — `ok: true` · `charter_gates: 24` · all checks `PASS`.
