...
| `entry_mode` | enum + conditional trigger | MUST be `telegram` or `web`; drives conditional fields | `VALIDATION_REQUIRED_FIELD_MISSING`, `VALIDATION_ENUM_INVALID` | inline | `POST /api/v2/registrations`, `POST /api/v2/auth/telegram/session`, `POST /api/v2/auth/web/session` |
