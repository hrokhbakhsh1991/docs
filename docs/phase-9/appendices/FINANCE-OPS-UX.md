...
| `(app)/settings/reconciliation-triage` | ❌                                                                                                 | Explorer + ledger adjust                           |
...
| Dashboard finance widget               | ❌                                                                                                 | KPI strip → hub link                               |
...
| **S9.7-R2** | Prepayment recording API · booking wallet panel · `PaymentSchedule` persistence · schedule generator         | stretch — CP marked optional for 9.8 |
...
```text
apps/web/app/finance/
  page.tsx                         # server gate · isFinanceRouteAllowed
  finance-command-center.tsx       # tab shell (overview · payments · receipts · ledger)

apps/web/src/finance/
  finance-nav-access.ts            # shouldShowFinanceNav · parseFinanceTab

apps/api/src/denali-finance/
  finance.routes.ts                # HTTP handlers (R1 consolidated)
  finance.service.ts
...
| CP-9.7-06 | Reports summary matches legacy counts     | R1    | API spec     |
| CP-9.7-07 | Manual payment + receipt E2E              | R1    | integration  |
| CP-9.7-08 | Receipt approve posts ledger outbox       | R1    | integration  |
| CP-9.7-09 | Dashboard finance widget KPIs             | R1    | web spec     |
| CP-9.7-10 | Record prepayment updates wallet          | R2    | API spec     |
| CP-9.7-11 | Invoice balance due correct               | R2    | unit         |
| CP-9.7-12 | Generate schedule from template           | R2    | API spec     |
