...
async function createZibalPaymentIntentWithCredentials(
...
/** Tenant-scoped Zibal adapter using explicit merchant + callback credentials. */
export function createZibalPaymentGateway(
  idempotencyStore: IdempotencyKeyStore,
  merchantId: string,
  callbackUrl: string,
): IPaymentGateway {
  return {
    ...
    createPaymentIntent: (input) =>
      createZibalPaymentIntentWithCredentials(
        idempotencyStore,
        merchantId,
        callbackUrl,
        input,
