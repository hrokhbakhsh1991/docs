...
/** Accept `string` so unknown runtime API values never break rendering. */
...
export function BookingStatusBadge({ status }: { status: RegistrationStatus }) {
  return (
    <Badge className="shared-booking-badge" variant={bookingStatusVariant(status)}>{bookingStatusLabel(status)}</Badge>
  );
}
...
export function PaymentStatusBadge({ payment }: { payment: RegistrationPaymentStatus | string }) {
  ...
  return (
    <Badge className="shared-booking-badge" variant={paymentVariant(raw)}>{paymentStatusLabel(raw)}</Badge>
  );
}
