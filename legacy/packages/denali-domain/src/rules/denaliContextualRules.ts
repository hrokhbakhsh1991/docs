...
import { getDenaliFormPathValue } from "../adapters/denaliFormPathUtils";
import {
...
import { getWorkspaceUiCapabilityFlags } from "@repo/shared-contracts";
...
export function evaluateDenaliContextualRule(
  ...
  switch (rule.kind) {
    ...
    case "capability": {
      ...
      const capabilities = getWorkspaceUiCapabilityFlags(profile);
      if (rule.flag === "canDefineCustomServices") {
        return capabilities.canDefineCustomServices ?? false;
      }
      return false;
    }
