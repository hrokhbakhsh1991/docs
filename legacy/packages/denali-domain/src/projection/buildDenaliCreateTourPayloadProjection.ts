...
import { isTransientMediaUrl } from "./isTransientMediaUrl";
...
import {
  ...
  YMD_RE,
} from "./wizardMapperHelpers";

type ApiTourType = "mountain" | "city" | "desert" | "nature" | "cultural";
type TourTripDetails = Record<string, unknown>;
...
export type { DenaliCreateTourPayloadProjection } from "./wizardMapperHelpers";
...
type DenaliSegmentActivitiesPayload = Array<Record<string, unknown>>;
...
function mapCanonicalTransport(transport: DenaliCanonicalTourModel["transport"]): {
  ...
  if (isDenaliOrganizedTransportWithPersonalCarOption(transport.mode)) {
    ...
    const modes: string[] = allowPersonal ? [apiPrimary, "private_car"] : [apiPrimary];
    const dongVisible = isDenaliTransportDongAmountVisible({
      mode: transport.mode,
      allowPersonalCar: transport.allowPersonalCar,
    });
      ...
      ? canonical.program.itinerary
          .map((row) => {
            ...
            return {
              ...
              photos: (row.photos ?? [])
                .filter((p) => p.id && p.url && !isTransientMediaUrl(p.url))
                .map((p) => ({
                  id: p.id,
                  url: p.url,
                  filename: p.filename,
...
export function buildDenaliCreateTourPayloadProjection(
  ...
  const tripDetailsWithOverview = mergeDenaliWizardOverviewIntoTripDetails(
    projection.tripDetails as TourTripDetails | undefined,
    {
      customServiceLabels,
