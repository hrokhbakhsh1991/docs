#!/usr/bin/env node
/**
 * wallet-core boundary — package-local hard isolation (Phase 2B).
...
const FORBIDDEN_SPEC_RULES = [
  ...
  },
  {
    re: /^@apps\/api\b/,
    dependency: "@apps/api",
    reason: "wallet-core must not import the API package",
  },
  {
    re: /(^|\/)apps\/api(\/|$)/,
    dependency: "apps/api",
    reason: "wallet-core must not import host application paths",
  },
  {
    re: /^@app-tour\/finance-core(\/|$)/,
    dependency: "@app-tour/finance-core",
    reason: "wallet-core is a separate bounded context from finance-core",
  },
  {
  ...
  },
  {
    ...
    reason: "wallet-core must remain workspace-agnostic",
  },
  {
    re: /(^|\/)packages\/workspaces\//,
    dependency: "packages/workspaces/*",
    reason: "workspace implementations must not enter wallet-core",
  },
  {
    re: /(^|\/)apps\/(portal|web|marketing)(\/|$)/,
    dependency: "apps/portal|web|marketing",
    reason: "UI packages must not enter wallet-core",
  },
  {
    re: /\.generated(\.|$)/,
...
if (!fs.existsSync(CORE_SRC)) {
  console.error("guard-wallet-core-boundary: FAIL — packages/wallet-core/src missing");
  process.exit(1);
}
...
if (violations.length > 0) {
  console.error("guard-wallet-core-boundary: FAIL");
...
console.log("guard-wallet-core-boundary: PASS");
