...
describe("createFreshStarterPlugin()", () => {
  ...
  it("satisfies WorkspacePlugin structural guard", () => {
    assert.equal(isWorkspacePlugin(createFreshStarterPlugin()), true);
  });

  it("serves starter workspace type only", () => {
    const plugin = createFreshStarterPlugin();
    assert.deepEqual(plugin.supportedWorkspaceTypes, [STARTER_WORKSPACE_TYPE]);
    ...
    assert.equal(
      resolveWorkspacePluginIdForType("starter", DEFAULT_WORKSPACE_TYPE_BINDINGS),
      STARTER_WORKSPACE_PLUGIN_ID,
    );
  ...
  it("includes platform-primary workspace theme preset for harness demos", () => {
    const plugin = createFreshStarterPlugin();
    assert.ok(plugin.theme);
    assert.equal(plugin.theme.id, "starter-primary-accent");
    assert.equal(
      plugin.theme.cssVariables["--ws-color-accent"],
      "var(--color-primary)",
    );
