 ...
 * Domain validation and booking orchestration stay in workspace packages.
 */
...
export function readWorkspaceCanonicalCapacityByPath(
  canonical: { readonly data: unknown },
  path: readonly string[],
): number | null {
  if (path.length === 0) {
    return null;
  }
  let cursor: unknown = canonical.data;
 ...
 * Product-specific fields (notes, nationalId, transport, …) stay in W.
 */
