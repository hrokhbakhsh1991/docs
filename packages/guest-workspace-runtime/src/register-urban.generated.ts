...
export async function registerWorkspacePluginURBANFromManifest(): Promise<void> {
  ...
  registerWorkspaceMemberPortalRenderers(plugin.id, plugin.capabilities?.memberPortalRenderers);
  const { urbanCatalogRegistrationFlowSurface } =
    await import("@app-tour/workspace-urban/host/catalog-registration-flow");
  const { UrbanIntakeStep, UrbanDoneStep } =
    await import("@app-tour/workspace-urban/host/catalog-registration-flow/react");
  const {
