...
export async function registerWorkspacePluginHARBORFromManifest(): Promise<void> {
  ...
  registerWorkspaceMemberPortalRenderers(plugin.id, plugin.capabilities?.memberPortalRenderers);
  const { harborCatalogRegistrationFlowSurface } =
    await import("@app-tour/workspace-harbor/host/catalog-registration-flow");
  const { HarborIntakeStep, HarborDoneStep } =
    await import("@app-tour/workspace-harbor/host/catalog-registration-flow/react");
  const {
