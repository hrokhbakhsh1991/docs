...
import {
  FINANCE_WS4_LEDGER_ACCOUNTS,
  financeWs4BookingWalletId,
} from "./chart-of-accounts";

/**
 * Fake WS4 ledger policy — owns CoA posting; does not import Denali finance helpers.
 * Phase 3B capture domainEventId formula stays host-stable.
 */
export class FinanceWs4LedgerPolicyAdapter implements FinanceLedgerPolicyPort {
  buildPaymentCaptureJournal(input: BuildPaymentCaptureJournalInput): FinanceLedgerCapturePlan {
    ...
    const { journalId, lines } = postWs4DoubleEntry({
      tenantId: input.tenantId,
      debitAccount: FINANCE_WS4_LEDGER_ACCOUNTS.OPERATOR_CASH_CLEARING,
      creditAccount: financeWs4BookingWalletId(input.registrationId),
      ...
      metadata: {
        ...
        workspaceFinancePlugin: "finance-ws4",
      },
    });
    return {
      journalId,
...
function stableLedgerIdentifiers(seed: string): {
  ...
  const id = seed.trim();
  return {
    journalId: deterministicUuidFromSeed(`ws4-payment-ledger:journal:${id}`),
    debitLineId: deterministicUuidFromSeed(`ws4-payment-ledger:debit:${id}`),
    creditLineId: deterministicUuidFromSeed(`ws4-payment-ledger:credit:${id}`),
  };
}

function postWs4DoubleEntry(input: {
  tenantId: string;
  debitAccount: string;
  ...
  if (!tenantId || !debitAccount || !creditAccount) {
    throw new Error("FINANCE_WS4_LEDGER_REQUIRED: tenantId and accounts are required");
  }
  if (debitAccount === creditAccount) {
    throw new Error("FINANCE_WS4_LEDGER_ACCOUNTS_DISTINCT: debitAccount !== creditAccount");
  }
  if (!/^\d+$/.test(input.amount_minor.trim()) || BigInt(input.amount_minor.trim()) <= 0n) {
    throw new Error("FINANCE_WS4_LEDGER_AMOUNT_INVALID: amount_minor must be a positive integer string");
  }
