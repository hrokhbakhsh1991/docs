...
import {
  FINANCE_WS6_LEDGER_ACCOUNTS,
  financeWs6BookingWalletId,
} from "./chart-of-accounts";

/**
 * Fake WS6 ledger policy — owns CoA posting; host-stable capture domainEventId formula.
 */
export class FinanceWs6LedgerPolicyAdapter implements FinanceLedgerPolicyPort {
  buildPaymentCaptureJournal(input: BuildPaymentCaptureJournalInput): FinanceLedgerCapturePlan {
    ...
    const { journalId, lines } = postWs6DoubleEntry({
      tenantId: input.tenantId,
      debitAccount: FINANCE_WS6_LEDGER_ACCOUNTS.OPERATOR_CASH_CLEARING,
      creditAccount: financeWs6BookingWalletId(input.registrationId),
      ...
      metadata: {
        ...
        workspaceFinancePlugin: "finance-ws6",
      },
    });
    return {
      journalId,
...
function stableLedgerIdentifiers(seed: string): {
  ...
  const id = seed.trim();
  return {
    journalId: deterministicUuidFromSeed(`ws6-payment-ledger:journal:${id}`),
    debitLineId: deterministicUuidFromSeed(`ws6-payment-ledger:debit:${id}`),
    creditLineId: deterministicUuidFromSeed(`ws6-payment-ledger:credit:${id}`),
  };
}

function postWs6DoubleEntry(input: {
  tenantId: string;
  debitAccount: string;
  ...
  if (!tenantId || !debitAccount || !creditAccount) {
    throw new Error("FINANCE_WS6_LEDGER_REQUIRED: tenantId and accounts are required");
  }
  if (debitAccount === creditAccount) {
    throw new Error("FINANCE_WS6_LEDGER_ACCOUNTS_DISTINCT: debitAccount !== creditAccount");
  }
  if (!/^\d+$/.test(input.amount_minor.trim()) || BigInt(input.amount_minor.trim()) <= 0n) {
    throw new Error("FINANCE_WS6_LEDGER_AMOUNT_INVALID: amount_minor must be a positive integer string");
  }
