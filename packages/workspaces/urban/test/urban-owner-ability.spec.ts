/**
 * Phase 8.1 — urban owner ability contract (SDK-8.1-01..08)
 * Authority: docs/phase-8/subphases/8.1-single-owner-auth.md §5 · DEC-P8-004 superseded by Phase 10.5
 * Implementation: packages/workspaces/urban/src/auth/urban-owner-auth.ts
...
import { isWorkspaceOwner } from "@app-tour/workspace-sdk/auth";
import { canPerformUrbanOwnerMutation } from "../src/auth/urban-owner-auth";
...
describe("Phase 8.1 urban owner ability (Phase 10.5)", () => {
  it("SDK-8.1-01 owner can mutate urban.settings.update on urban workspace", () => {
    const authz = buildTenantAuthz(activeAuth("owner"));
    expect(
      canPerformUrbanOwnerMutation(authz, URBAN_TENANT_ID, "urban.settings.update", "urban")
    ).toBe(true);
  });
  ...
  it("SDK-8.1-04 owner can declare urban.catalog.publish capability", () => {
    const authz = buildTenantAuthz(activeAuth("owner"));
    expect(
      canPerformUrbanOwnerMutation(authz, URBAN_TENANT_ID, "urban.catalog.publish", "urban")
    ).toBe(true);
  });
  ...
  it("SDK-8.1-06 owner on starter workspace cannot mutate urban.settings.update", () => {
    ...
    expect(
      canPerformUrbanOwnerMutation(authz, STARTER_TENANT_ID, "urban.settings.update", "starter")
    ).toBe(false);
  });
