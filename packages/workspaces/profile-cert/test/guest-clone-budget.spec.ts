...
describe("profile-cert guest clone detector lite", () => {
  it("keeps HTTP surface tiny vs Denali-scale forests", () => {
    const httpFiles = listTsFiles(join(root, "src/http"));
    assert.ok(
      httpFiles.length <= 6,
      `expected ≤6 profile-cert http modules, got ${httpFiles.length}`,
    );
  });

  it("does not import workspace-denali from profile-cert src", () => {
