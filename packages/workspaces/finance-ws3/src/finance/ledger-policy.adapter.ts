...
import {
  FINANCE_WS3_LEDGER_ACCOUNTS,
  financeWs3BookingWalletId,
} from "./chart-of-accounts";

/**
 * Fake WS3 ledger policy — owns CoA posting; does not import Denali finance helpers.
 * Phase 3B capture domainEventId formula stays host-stable.
 */
export class FinanceWs3LedgerPolicyAdapter implements FinanceLedgerPolicyPort {
  buildPaymentCaptureJournal(input: BuildPaymentCaptureJournalInput): FinanceLedgerCapturePlan {
    ...
    const { journalId, lines } = postWs3DoubleEntry({
      tenantId: input.tenantId,
      debitAccount: FINANCE_WS3_LEDGER_ACCOUNTS.OPERATOR_CASH_CLEARING,
      creditAccount: financeWs3BookingWalletId(input.registrationId),
      ...
      metadata: {
        ...
        workspaceFinancePlugin: "finance-ws3",
      },
    });
    return {
      journalId,
...
function stableLedgerIdentifiers(seed: string): {
  ...
  const id = seed.trim();
  return {
    journalId: deterministicUuidFromSeed(`ws3-payment-ledger:journal:${id}`),
    debitLineId: deterministicUuidFromSeed(`ws3-payment-ledger:debit:${id}`),
    creditLineId: deterministicUuidFromSeed(`ws3-payment-ledger:credit:${id}`),
  };
}

function postWs3DoubleEntry(input: {
  tenantId: string;
  debitAccount: string;
  ...
  if (!tenantId || !debitAccount || !creditAccount) {
    throw new Error("FINANCE_WS3_LEDGER_REQUIRED: tenantId and accounts are required");
  }
  if (debitAccount === creditAccount) {
    throw new Error("FINANCE_WS3_LEDGER_ACCOUNTS_DISTINCT: debitAccount !== creditAccount");
  }
  if (!/^\d+$/.test(input.amount_minor.trim()) || BigInt(input.amount_minor.trim()) <= 0n) {
    throw new Error("FINANCE_WS3_LEDGER_AMOUNT_INVALID: amount_minor must be a positive integer string");
  }
