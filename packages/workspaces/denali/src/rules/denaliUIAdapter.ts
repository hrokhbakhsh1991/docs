...
import type { DenaliCanonicalTourModel } from "../types/legacy/repo-types-denali";
...
export function getDenaliUIAdapterMetadata(
  input: DenaliUIAdapterInput
): DenaliUIFieldMetadata | null {
...
function buildDenaliUIContext(
  ...
  return {
    ...
    ruleModel,
    stepUI: (step) => (ruleModel == null ? null : deriveDenaliUIFieldMetadata(ruleModel, step)),
    ...
    isRequiredInModel: (path, f) =>
      isDenaliFieldRequiredInModel(ruleModel, path, f ?? form, options),
...
export function getDenaliUIFromForm(
  ...
  const ruleModel = basics == null ? null : ruleSet[basics.category][basics.duration];
