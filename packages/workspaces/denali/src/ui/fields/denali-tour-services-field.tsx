...
import {
  ...
  UserRoundIcon,
  XIcon,
} from "../components/icons/tour-service-icons";
    ...
    >
      <header className="denali-tour-services__column-header">
        <span className="denali-tour-services__column-icon" aria-hidden>
          {bucket === "included" ? <CheckCircleIcon /> : <UserRoundIcon />}
        </span>
      ...
      <div className="denali-tour-services__add-row">
        <Input
          ...
          aria-label={addLabel}
        />
        <Button type="button" variant="secondary" data-testid={addTestId} onClick={() => tryAdd(draftLabel)}>
          {addLabel}
        </Button>
      </div>
