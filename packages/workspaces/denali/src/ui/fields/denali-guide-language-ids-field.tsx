...
export function DenaliGuideLanguageIdsField({
  ...
  const pickerLabels = useMemo(
    () => ({
      collapsePicker: t("composites.guideLanguages.collapsePicker"),
      expandPicker: t("composites.guideLanguages.expandPicker"),
      selectedCount: (count: number) => t("composites.guideLanguages.selectedCount", { count }),
      overflowCount: (count: number) => t("composites.guideLanguages.overflowCount", { count }),
      removeItem: (name: string) => t("composites.guideLanguages.removeLanguage", { name }),
      searchLabel: t("composites.guideLanguages.searchLabel"),
      searchPlaceholder: t("composites.guideLanguages.searchPlaceholder"),
      searchEmpty: t("composites.guideLanguages.searchEmpty"),
    }),
    [t]
  );

  const writeSelected = (next: readonly string[]) => {
  ...
  const toggleLanguage = (languageId: string) => {
    const next = selectedSet.has(languageId)
  ...
  const removeLanguage = (languageId: string) => {
    writeSelected(selected.filter((id) => id !== languageId));
  };
  ...
  return (
    <div
      ...
      data-operator-guide-language-picker
    ...
    >
      ...
      {!loading && languages.length > 0 ? (
        <DenaliCatalogMultiPicker
          ...
          labels={pickerLabels}
          dataAttribute="data-operator-guide-language-picker"
        />
      ) : null}
    </div>
  );
