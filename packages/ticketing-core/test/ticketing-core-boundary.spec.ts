/**
 * TKT-001 Phase 2 — ticketing-core boundary and portability proof.
 */
...
describe("ticketing-core boundary", () => {
  it("layout has domain and application layers", () => {
    assert.equal(existsSync(join(SRC, "domain")), true);
    assert.equal(existsSync(join(SRC, "application/create-ticket.ts")), true);
  });
  ...
  it("source tree has no forbidden imports", () => {
    ...
    for (const relative of files) {
      ...
      assert.doesNotMatch(src, /from ["']@app-tour\/ticketing-http-contracts/);
  ...
  it("exports pure services from package root in plain Node", async () => {
    ...
    assert.equal(typeof mod.createTicket, "function");
    assert.equal(typeof mod.canTransitionTicketStatus, "function");
    assert.equal(Array.isArray(mod.TICKETING_ERROR_CODES), true);
  });
});
