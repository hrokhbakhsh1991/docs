/**
 * Static Iranian mountaineering landmarks — platform geocoding fallback.
 * Thin Shell Phase 4bi: lives outside apps/web (not product-workspace knowledge).
 */
...
/** Normalized hit shape compatible with admin geocoding merge. */
export type IranMountainGeocodingHit = {
  readonly displayName: string;
  readonly addressText: string;
  readonly latitude: number;
  readonly longitude: number;
...
function landmarkMatchesQuery(
  landmark: IranMountainLandmark,
...
export function searchIranMountainLandmarks(
  query: string,
  limit = 6
): IranMountainGeocodingHit[] {
  const normalizedQuery = normalizeGeocodingQuery(query);
  if (normalizedQuery.length < 2) {
    return [];
  }
  const out: IranMountainGeocodingHit[] = [];
