...
export async function POST(req: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    upstream = await fetchTicketsUpstream(context.host, `/member/tickets/${ticketId}/messages`, {
      method: "POST",
      body,
      idempotencyKey,
    });
  ...
  if (!upstream.ok) {
    ...
    const code = readMemberTicketsBffErrorCode(errorBody) ?? "TICKET_MESSAGE_FAILED";
    return jsonTicketsError(
      code,
      upstream.status,
      localizeMemberTicketsBffError(code, "Reply failed"),
    );
  }
