#!/usr/bin/env node
/**
 * Starts API + Portal for guest-club portal visual smoke (SMK-PTL-VIS-guest-01).
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() > deadline) {
        reject(new Error(`smoke-portal-guest-club-e2e-servers: timeout waiting for ${url}`));
        return;
      }
      setTimeout(tick, 500);
    };
...
const portalSmokeHost =
  process.env.SMOKE_PORTAL_BASE_URL?.trim() || "http://guest-club.portal.localhost:3003";
...
const portalEnv = {
  ...
  MARKETING_PUBLIC_BASE_URL: "http://guest-club.localhost:3002",
};
...
void waitForUrl("http://127.0.0.1:3001/health")
  ...
  .then(async () => {
    console.log("smoke-portal-guest-club-e2e-servers: API + portal ready");
    await new Promise(() => {});
  })
  .catch((error) => {
    console.error(error);
