/**
 * MEG-UX — BEFORE capture (home only, pre-remediation panel).
 */
...
test.describe("MEG-UX portal before home capture", () => {
  test("member home only", async ({ page }) => {
    const phone = `+1555${String(Date.now()).slice(-7)}`;
    await authenticatePortalMemberForEngagement(page, {
      phone,
      fullName: "UX Before Capture",
    });
    ...
    for (const viewport of VIEWPORTS) {
      ...
      await page.screenshot({
        path: `${ARTIFACT_DIR}/before-portal-member-home-${viewport.name}.png`,
        fullPage: true,
      });
    }
  });
