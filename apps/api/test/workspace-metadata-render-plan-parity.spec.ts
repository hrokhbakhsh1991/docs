...
export async function buildParityRenderPlans(): Promise<{
...
describe("workspace-metadata-render-plan-parity", () => {
  it("RP-01 same total visible field count per step", async () => {
    const { packagePlan, metadataPlan } = await buildParityRenderPlans();
  ...
  it("RP-04 denali seed retains denali.* composite ids", async () => {
    const { seedPayload } = await buildParityRenderPlans();
