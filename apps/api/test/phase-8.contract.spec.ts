...
function reportPlatformCoreDirtyWorktree(): void {
  ...
  if (status.length > 0) {
    console.warn(
      `REQ-P8 dirty-work drift (committed HEAD remains canonical):\n${status}`
    );
  }
}
...
function assertPlatformCoreMatchesTreeDigest(): void {
  ...
  assert.equal(
    committedHeadDigest,
    expectedDigest,
    `committed HEAD platform-core tree digest drift since phase-8 baseline (proof rev ${PHASE_8_GENERICITY_PROOF_REV})`
  );
}
