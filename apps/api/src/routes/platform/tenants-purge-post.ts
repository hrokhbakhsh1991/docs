...
import { purgePlatformTenant } from "../../platform/purge-platform-tenant.ts";
import {
  PlatformForbidden,
  PlatformUnauthorized,
} from "../../platform/platform.errors.ts";

export async function handlePlatformTenantsPurgePost(
  req: IncomingMessage,
  res: ServerResponse,
  tenantId: string
): Promise<void> {
  ...
  const deleted = await purgePlatformTenant({ tenantId, actorId: ctx.actorId });
  if (!deleted) {
    res.writeHead(403, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: "forbidden", code: "PURGE_NOT_ELIGIBLE" }));
    return;
  }
