...
import { cancelPlatformTenantOffboard } from "../../platform/cancel-platform-tenant-offboard.ts";
...
export async function handlePlatformTenantsCancelOffboardPost(
  req: IncomingMessage,
  res: ServerResponse,
  tenantId: string
): Promise<void> {
  ...
  const updated = await cancelPlatformTenantOffboard({ tenantId, actorId: ctx.actorId });
  if (!updated) {
