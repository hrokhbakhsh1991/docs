...
import { hasDenaliFinanceProcessedEvent } from "./denali-finance-processed-log";
...
export function createPrismaDenaliOutboxReader(tenantId: string): OutboxReader {
  return {
    async readPending(): Promise<readonly DenaliOutboxDomainEvent[]> {
      ...
      for (const row of rows) {
        ...
        if (await hasDenaliFinanceProcessedEvent(tenantId, mapped.domainEventId)) {
          continue;
        }
