...
import { lockRegistrationEntityForFinancialMutation } from "./lock-registration-for-financial-mutation";
 ...
 * Registration reads delegate to {@link REGISTRATIONS_READ_REPOSITORY_PORT} (**persistence only**).
...
export class TypeOrmRegistrationsApplicationService implements RegistrationsApplicationPort {
  private readonly logger = new Logger(TypeOrmRegistrationsApplicationService.name);
  ...
  async getRegistrationById(
    ...
    return this.toRegistrationResponseFromDetail(registration);
  }
  ...
  async updatePaymentStatus(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (
        !registrationTenantMatchesActorScope(actorRole, trustedTenantId, peek.tenantId)
      ) {
        throw new NotFoundException({
          error: {
            code: "RESOURCE_NOT_FOUND",
  ...
  private toRegistrationResponseFromDetail(
  ...
  async transitionRegistrationForPayment(
    ...
    return this.transitionRegistrationForPaymentEntity(manager, reg, targetStatus, actorId).then(
