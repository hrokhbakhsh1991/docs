      ...
      try {
        await this.tenantDbContext.runInTenantScope(tenantId, async (manager) => {
          ...
          if (row.eventType === OUTBOX_EVENT_TYPE_IDENTITY_EMAIL_VERIFICATION_SEND) {
            ...
            } else if (row.eventType === "payment.captured" || row.eventType === "payment.refunded") {
              // Dynamic resolution of listeners to avoid circular module dependencies
              try {
                const PaymentLedgerSyncListener = (await import("../../finance/repositories/payment-ledger-sync.listener")).PaymentLedgerSyncListener;
                const ledgerSync = this.moduleRef.get(PaymentLedgerSyncListener, { strict: false });
                await ledgerSync.handle(manager, tenantId, payload);
              } catch (e) {
                this.logger.error("Failed to load or execute PaymentLedgerSyncListener", e);
                throw e;
              }

              if (row.eventType === "payment.captured") {
                try {
                  const PaymentCapturedListener = (await import("../../registrations/listeners/payment-captured.listener")).PaymentCapturedListener;
                  const paymentCaptured = this.moduleRef.get(PaymentCapturedListener, { strict: false });
                  await paymentCaptured.handle(manager, tenantId, payload);
                } catch (e) {
                  this.logger.error("Failed to load or execute PaymentCapturedListener", e);
                  throw e;
                }
              }
            } else if (row.eventType === "registration.created") {
              try {
                const RegistrationCreatedLedgerListener = (
                  await import("../../finance/listeners/registration-created-ledger.listener")
                ).RegistrationCreatedLedgerListener;
                const listener = this.moduleRef.get(RegistrationCreatedLedgerListener, { strict: false });
                await listener.handle(manager, tenantId, row.id, payload);
              } catch (e) {
                this.logger.error("Failed to load or execute RegistrationCreatedLedgerListener", e);
                throw e;
              }
            } else if (
              row.eventType === "registration.payment_updated"
            ) {
              try {
                const RegistrationPaymentUpdatedLedgerListener = (
                  await import("../../finance/listeners/registration-payment-updated-ledger.listener")
                ).RegistrationPaymentUpdatedLedgerListener;
                const listener = this.moduleRef.get(RegistrationPaymentUpdatedLedgerListener, {
                  strict: false,
                });
                await listener.handle(manager, tenantId, row.id, payload);
              } catch (e) {
                this.logger.error("Failed to load or execute RegistrationPaymentUpdatedLedgerListener", e);
                throw e;
              }
            } else if (
              row.eventType === "booking.finalized" ||
              row.eventType === "booking.finalization.booking_confirmed"
            ) {
              try {
                const BookingFinalizedLedgerListener = (
                  await import("../../finance/listeners/booking-finalized-ledger.listener")
                ).BookingFinalizedLedgerListener;
                const listener = this.moduleRef.get(BookingFinalizedLedgerListener, { strict: false });
                await listener.handle(manager, tenantId, row.id, payload);
              } catch (e) {
                this.logger.error("Failed to load or execute BookingFinalizedLedgerListener", e);
                throw e;
              }
            }
