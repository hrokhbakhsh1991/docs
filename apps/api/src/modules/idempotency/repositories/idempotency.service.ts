...
export class IdempotencyService {
  ...
  async findByKey(key: string): Promise<IdempotencyKeyEntity | null> {
    ...
    const record = await this.idempotencyRepository.findOne({ where: { tenantId, key } });
    if (record) {
      this.assertRecordTenantScope(record, tenantId);
    }
    return record;
  }
  ...
  async deleteExpiredWithManager(manager: EntityManager): Promise<number> {
    const result = await manager.delete(IdempotencyKeyEntity, {
      expiresAt: LessThan(new Date()),
    });
    return result.affected ?? 0;
  }
  ...
  private assertRecordTenantScope(
    record: Pick<IdempotencyKeyEntity, "tenantId">,
    tenantId: string
  ): void {
    if (record.tenantId.trim().toLowerCase() !== tenantId.trim().toLowerCase()) {
  ...
  private async findRecord(
    ...
    const record = await manager.findOne(IdempotencyKeyEntity, {
      where: { tenantId, key }
    });
    if (record) {
      this.assertRecordTenantScope(record, tenantId);
