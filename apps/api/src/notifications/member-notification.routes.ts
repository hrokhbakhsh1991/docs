...
function assertMemberCanMutate(auth: TenantAuthContext): void {
  if (auth.role === "viewer") {
    throw new Error("MEMBER_VIEWER_READ_ONLY");
  }
}
...
export async function handleMemberListNotifications(
  req: IncomingMessage,
  res: ServerResponse,
): Promise<void> {
...
export async function handleMemberUnreadNotificationCount(
  req: IncomingMessage,
  res: ServerResponse,
): Promise<void> {
  await memberHandler(req, res, async (auth) => {
...
export async function handleMemberMarkNotificationRead(
  req: IncomingMessage,
  res: ServerResponse,
  ...
  await memberHandler(req, res, async (auth) => {
    ...
    const row = await markMemberNotificationRead({
      tenantId: auth.tenantId,
      notificationId,
      userId: auth.userId,
    });
    if (row === null) {
      sendNotificationJson(res, 404, { ok: false, code: "MEMBER_NOTIFICATION_NOT_FOUND" });
      return;
    }
...
export async function handleMemberMarkAllNotificationsRead(
  req: IncomingMessage,
  res: ServerResponse,
): Promise<void> {
  await memberHandler(req, res, async (auth) => {
