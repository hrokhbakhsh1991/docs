...
import { OPERATOR_WIZARD_PATH, requireOperatorSessionWeb } from "../src/admin/require-operator-session";
...
describe("admin-shell-access.spec.ts — Phase 9.2", () => {
  ...
  it("CP-9.2-05 admin nav excludes owner-only sections (DEC-P9-018)", () => {
    ...
    assert.equal(items.some((item) => item.href === "/users"), false);
    assert.equal(items.some((item) => item.href === "/settings"), false);
    assert.equal(items.some((item) => item.href === "/finance"), false);
  });
