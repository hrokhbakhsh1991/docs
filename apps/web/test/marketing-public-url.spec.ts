...
describe("resolve-marketing-public-url", () => {
  it("WEB-MKT-01 dev club admin host maps to canonical marketing origin (no shop prefix)", () => {
    assert.equal(
  ...
  it("WEB-MKT-01b club portal ingress maps to club marketing apex", () => {
    assert.equal(
  ...
  it("WEB-MKT-02 tour detail path", () => {
    assert.equal(
      ...
      "http://urban.localhost:3002/tours/tour-1"
    );
  });
  ...
  it("WEB-MKT-03 tours list with cursor", () => {
    assert.equal(
      ...
      "http://urban.localhost:3002/tours?cursor=cursor-1"
    );
  });
