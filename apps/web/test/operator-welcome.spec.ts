 ...
 *
 * Sync gates read the wizardCreate warm cache — seed before assert (fail-closed when cold).
 */
...
afterEach(() => {
  seedWizardCreate(DENALI_PLUGIN_ID, { extendedChrome: false });
  seedWizardCreate("urban", { extendedChrome: false });
});

describe("operator-welcome.spec.ts", () => {
  it("CP-WELCOME-03 gate active for denali owner only", () => {
    seedWizardCreate(DENALI_PLUGIN_ID, { extendedChrome: true });
    seedWizardCreate("urban", { extendedChrome: false });
