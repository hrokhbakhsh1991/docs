#!/usr/bin/env node
/**
 * Starts API + Portal for Denali catalog registration smoke (SMK-DREG-01).
 * Registration intake lives on apps/portal (DEC-P11-014).
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() > deadline) {
        reject(new Error(`smoke-denali-catalog-e2e-servers: timeout waiting for ${url}`));
        return;
      }
      setTimeout(tick, 500);
    };
