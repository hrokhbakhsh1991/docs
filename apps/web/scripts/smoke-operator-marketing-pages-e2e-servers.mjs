#!/usr/bin/env node
/**
 * API + Denali operator web + marketing for MKP-001 Playwright smoke.
 * Memory driver; tenant …000003 on denali.admin.localhost + denali.localhost.
 */
...
const readinessPort = Number(process.env.MKP_SMOKE_READY_PORT?.trim() || "3015");
...
async function waitForPortFree(port, timeoutMs = 10_000) {
  ...
  throw new Error(`smoke-operator-marketing-pages: port ${port} still in use`);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() > deadline) {
        reject(new Error(`smoke-operator-marketing-pages: timeout waiting for ${url}`));
        return;
      }
...
try {
  console.warn("smoke-operator-marketing-pages: freeing ports 3000–3002");
  freePort(3000);
  freePort(3001);
  freePort(3002);
  ...
  await warmMarketingPath("/");

  readinessReady = true;
  console.log("smoke-operator-marketing-pages: API + denali admin web + marketing ready");
  await keepAlive();
} catch (error) {
  console.error(error);
  shutdown("SIGTERM");
