#!/usr/bin/env node
/**
 * API + Web for Denali operator wallet BQC (engagement fixtures + default wallet seed).
 */
...
import { freePort } from "../../../scripts/lib/smoke-cert-server-utils.mjs";
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() > deadline) {
        reject(new Error(`smoke-operator-wallet-bqc-servers: timeout waiting for ${url}`));
        return;
      }
      setTimeout(tick, 500);
    };
...
function runSeed(scriptPath) {
  const seed = spawnSync("pnpm", ["exec", "tsx", scriptPath], {
    cwd: apiDir,
    env: seedEnv,
    stdio: "inherit",
  });
...
runSeed("scripts/seed-operator-engagement-e2e-fixtures.ts");
runSeed("scripts/seed-denali-default-wallet.ts");
...
await waitForUrl(`http://${denaliSmokeCanonicalHost}/wallet`);

console.log("smoke-operator-wallet-bqc-servers: API + web + wallet seed ready");
