...
export function rebaseCategoryDraftChangeOntoLatest(
  latest: TourWizardDraft,
  incoming: TourWizardDraft
): TourWizardDraft {
...
function mergeIncomingDraftDiffOntoLatest(
  ...
  return { data: merged as TourWizardDraft["data"] };
}
...
export function resolveDenaliTourKindPickerOpen(hasSelection: boolean): boolean {
  return true;
}
