...
import { submitDenaliRegistrationAction } from "@/denali/submit-denali-registration.server";
import { submitUrbanRegistrationAction } from "@/urban/submit-urban-registration.server";
...
export function PublicCatalogRegistrationFlow({
  ...
  async function submitIntake(event: FormEvent): Promise<void> {
    ...
    try {
      const result =
        workspace === "denali"
          ? await submitDenaliRegistrationAction({
              tenantId,
              tourId,
              email,
              fullName: name,
          ...
          : await submitUrbanRegistrationAction({
              tenantId,
              tourId,
              email,
              fullName: name,
      ...
      if (!result.ok) {
        setIntakeError(resolveError(result.code));
        return;
      }
      setStep("done");
    } catch {
