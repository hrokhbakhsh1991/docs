    ...
    <DenaliFlatEditPageView
      ...
      slots={{
        ...
        renderReady: ({ core: readyCore, detail, tourId: readyTourId }) => {
          const localizedError = resolveTourErrorMessage(
            tErrors,
            readyCore.error ?? readyCore.submitError
          );
          ...
          return (
            <div className="mx-auto max-w-3xl space-y-6" data-testid={TOUR_EDIT_TEST_IDS.page}>
              ...
              <div className="space-y-2">
                <DraftSyncChrome
                  status={readyCore.draftSync.status}
                  schemaIssues={readyCore.draftSync.schemaIssues}
                  navLocked={readyCore.draftSync.navLocked}
                  pendingDraft={readyCore.draftSync.pendingDraft}
                  conflictReloadNotice={readyCore.draftSync.conflictReloadNotice}
                  onRetry={() => void readyCore.draftSync.retry()}
                  onFlush={() => void readyCore.draftSync.flush()}
                  onApplyPending={readyCore.draftSync.applyDraft}
                  onDiscardPending={() => {
                    if (readyCore.draftSync.pendingDraft != null) {
                      readyCore.draftSync.setData(readyCore.draftSync.pendingDraft.data, {
                        source: "remote",
                      });
                    }
                  }}
                  ...
                  canRevertQuarantine={readyCore.draftSync.canRevertQuarantine}
                  onRevertQuarantine={readyCore.draftSync.revertToLastValid}
                />
