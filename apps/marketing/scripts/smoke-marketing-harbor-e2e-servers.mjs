#!/usr/bin/env node
/**
 * Starts API + Portal + Marketing for harbor G1 Playwright smoke (DG-4.7 / SMK-MKT-HARBOR-01).
 * Marketing catalog → portal register CTA.
 */
...
const harborSmokeSeedTourTitle = "Harbor evening sail";
const harborSmokeSeedTourId = "00000000-0000-4000-8000-000000000521";
  ...
  return new Promise((resolve, reject) => {
    ...
    const retry = () => {
      if (Date.now() > deadline) {
        reject(new Error(`smoke-marketing-harbor-e2e-servers: timeout waiting for ${url}`));
        return;
      }
      setTimeout(tick, 500);
    };
...
async function probeHarborSmokeSeedReady() {
          ...
          try {
            ...
            resolve(
              Array.isArray(items) &&
                items.some(
                  (row) =>
                    typeof row?.title === "string" && row.title.trim() === harborSmokeSeedTourTitle,
                ),
            );
          } catch {
            resolve(false);
    ...
    const req = http.request(
      {
        hostname: "127.0.0.1",
        port: 3001,
        path: `/harbor/catalog/${harborSmokeSeedTourId}`,
        method: "GET",
        headers: { "x-tenant-id": harborSmokeTenantId },
      },
      (res) => {
...
try {
  if (forceFreshServers) {
    console.warn("smoke-marketing-harbor-e2e-servers: PW_NO_REUSE_SERVER=1 — freeing ports 3001–3003");
    freePort(3001);
    freePort(3002);
    freePort(3003);
  ...
  if (apiListening) {
    const seedReady = await probeHarborSmokeSeedReady();
    if (!seedReady) {
      console.warn(
        "smoke-marketing-harbor-e2e-servers: port 3001 busy without harbor smoke seed — restarting API",
      );
      freePort(3001);
      await new Promise((resolve) => setTimeout(resolve, 2_000));
      apiListening = false;
  ...
  if (!apiListening) {
    ...
    console.log("smoke-marketing-harbor-e2e-servers: reusing API on 3001");
  ...
  if (!portalListening) {
    ...
    console.log("smoke-marketing-harbor-e2e-servers: reusing portal on 3003");
  ...
  if (!marketingListening) {
    ...
    console.log("smoke-marketing-harbor-e2e-servers: reusing marketing on 3002");
    await waitForUrl(`http://${marketingSmokeHost}/health`, 30_000);
  }
  ...
  await new Promise((resolve, reject) => {
    ...
    req.on("error", reject);
    req.write(JSON.stringify({ phone: "+15550009921" }));
    req.end();
  }).catch((error) => {
    console.warn("smoke-marketing-harbor-e2e-servers: request-otp warm skipped:", error.message);
  });

  console.log("smoke-marketing-harbor-e2e-servers: API + portal + marketing ready (harbor G1)");
  await keepAlive();
} catch (error) {
  console.error(error);
  shutdown("SIGTERM");
